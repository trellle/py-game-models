...
def main() -> None:
    ...
    for key in content:
        race, _ = Race.objects.get_or_create(name=content[key]["race"]["name"],
                                             description=content[key]["race"]["description"])
        ...
        if content[key].get("guild"):
            guild, _ = Guild.objects.get_or_create(name=content[key]["guild"]["name"],
